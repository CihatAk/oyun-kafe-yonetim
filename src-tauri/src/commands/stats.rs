...
pub fn calculate_live_fee(station_id: String, payment_method: String, state: State<AppState>) -> Result<serde_json::Value, String> {
    let conn = state.db.lock().map_err(|e| format!("DB kilitlenemedi: {}", e))?;
    let (start_str, paused_at, total_paused, extra_controllers, rate_type, st_type, extra_secs, extra_since): (String, Option<String>, i64, i64, String, String, f64, Option<String>) = conn.query_row(
        "SELECT a.start_time, a.paused_at, COALESCE(a.total_paused_seconds,0), COALESCE(a.extra_controllers,0), COALESCE(a.rate_type,'nakit'), COALESCE(s.station_type,'standard'), COALESCE(a.extra_secs,0), a.extra_since FROM active_sessions a LEFT JOIN stations s ON a.station_id=s.id WHERE a.station_id=?1",
        params![station_id], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?, row.get(4)?, row.get(5)?, row.get(6)?, row.get(7)?)),
    ...
    let (dur_mins, dur_secs, fee, extra_fee, is_paused) =
        session_fee(&conn, &station_id, rate, &start_str, paused_at.as_deref(), total_paused, extra_controllers, extra_secs, extra_since.as_deref());
    let pricing = AppState::load_pricing_conn(&conn);
    let per_min = if rate == "nakit" {
    ...
    Ok(serde_json::json!({
        ...
        "extra_secs": extra_secs, "extra_since": extra_since,
